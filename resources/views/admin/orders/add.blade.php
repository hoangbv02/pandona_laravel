                ...
                <div class="bg-secondary rounded h-100 p-4">
                    ...
                    <form action="" method="post" enctype="multipart/form-data">
                            ...
                            <select class="form-select mb-3" name="id_kh" aria-label="Default select example"
                                ...
                                @if (!empty($users))
                                    @foreach ($users as $user)
                                        <option {{ old('id_kh') == $user->idkh ? 'selected' : '' }}
                                            value="{{ $user->idkh }}">
                                            {{ $user->tenkh }}</option>
                                    @endforeach
                                @endif
                            ...
                            <select class="form-select mb-3" name="id_sp" aria-label="Default select example"
                                ...
                                @if (!empty($products))
                                    @foreach ($products as $product)
                                        <option {{ old('id_sp') == $product->idsp ? 'selected' : '' }}
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" name="ngay_dat_hang" value="{{ old('ngay_dat_hang') }}"
                                class="form-control" id="exampleInputNgay">
                            @error('ngay_dat_hang')
                                <span class="error-message">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class=" form-floating mb-3">
                            <textarea class="form-control" name="dia_chi" id="floatingTextarea" style="height: 150px;">{{ old('dia_chi') }}</textarea>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select mb-3" name="trang_thai" aria-label="Default select example"
                                ...
                                <option>--Chọn--</option>
                                <option {{ old('trang_thai') == '0' ? 'selected' : '' }} value="0">Đã đặt hàng
                                </option>
                                <option {{ old('trang_thai') == '1' ? 'selected' : '' }} value="1">Đã giao hàng
                                </option>
                            </select>
                            @error('trang_thai')
                                <span class="error-message">{{ $message }}</span>
